...
def main():
    ...
    with tqdm(total=len(main_steps), desc="Overall Progress") as pbar:
        # Load configuration
        config = load_config('configs/config.yaml')
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[0]}")
        ...
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[1]}")
        ...
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[2]}")
        ...
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[3]}")
        ...
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[4]}")
        ...
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[5]}")

        # Evaluation
        evaluator = ModelEvaluator()
        results = evaluator.evaluate(best_model, X_test, y_test)
        pbar.update(1)
        pbar.set_description(f"Completed: {main_steps[6]}")
