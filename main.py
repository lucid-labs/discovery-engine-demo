...
def process_and_train_model(
    file_path: str, output_dir: str, config: dict, pbar: tqdm
) -> None:
    # Define main steps
    steps = [
    ...
    with tqdm(
        total=len(steps), desc=f"Processing {os.path.basename(file_path)}", leave=False
    ) as file_pbar:
        # Load and preprocess data
        ...
        nas = NeuralArchitectureSearch(
            ...
            output_shape=len(config["target_columns"]),
        )
        ...
        best_model.compile(
            optimizer=tf.keras.optimizers.Adam(best_params["learning_rate"]), loss="mse"
        )
        ...
        model_save_path = os.path.join(
            output_dir, os.path.splitext(relative_path)[0] + ".h5"
        )
