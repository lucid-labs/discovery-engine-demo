...
class FeatureTransformer:
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in self.feature_columns:
            transformed_df[f"{col}_rolling_mean_7d"] = (
                transformed_df[col].rolling(window=7).mean()
            )
            transformed_df[f"{col}_rolling_mean_30d"] = (
                transformed_df[col].rolling(window=30).mean()
            )
        ...
        scaled_features = self.scaler.fit_transform(
            transformed_df[self.feature_columns]
        )
    ...
    def get_feature_names(self) -> List[str]:
        return (
            ...
            + [f"{col}_log" for col in self.feature_columns]
