...
for root, dirs, files in os.walk(data_root):
    for file in files:
        if file.endswith(".csv"):
            ...
            borrow_rate_model_path = os.path.join(
                model_folder, f"{asset}_borrow_rate.keras"
            )
            lender_rate_model_path = os.path.join(
                model_folder, f"{asset}_lender_rate.keras"
            )

            # Train and save model for borrowRate
            train_and_save_model(
                df, feature_columns, "borrowRate", borrow_rate_model_path
            )

            # Train and save model for lenderRate
            train_and_save_model(
                df, feature_columns, "lenderRate", lender_rate_model_path
            )
